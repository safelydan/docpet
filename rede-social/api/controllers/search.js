...
export const searchPost = (req, res) =>{
    ...
    db.query('SELECT p.*, u.username, userImg FROM posts as p JOIN user as u ON (u.id = p.userId) WHERE p.post_desct LIKE ? OR u.username LIKE ORDER BY created_at DESC', [params], (error, data)=>{
        if (error){
            console.log(error)
            res.status(500).json({msg: 'erro no servidor'})
        }
