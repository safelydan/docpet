...
export const addFriendship = (req, res) => {
  ...
  db.query(
    "INSERT INTO codpet.friendship (follower_id, followed_id) VALUES ($1, $2)",
    ...
    (error) => {
      if (error) {
        ...
        res
          .status(200)
          .json({ msg: "Você está seguindo esta pessoa com sucesso" });
      }
    }
  );
};
  ...
  db.query(
    ...
    (error) => {
      if (error) {
        ...
        res
          .status(200)
          .json({ msg: "Você não está mais seguindo esse usuário" });
      }
    }
  );
};
  ...
  db.query(
    ...
    (error, data) => {
      if (error) {
        ...
        res
          .status(404)
          .json({ msg: "Nenhuma amizade encontrada para este usuário" });
      }
    }
  );
};
