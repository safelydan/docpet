...
function FriendshipTable() {
    ...
    <div className="fixed right-2">
      <div className={`text-gray-600 flex flex-col gap-4 ${isMenuOpen ? 'bg-white shadow-md' : 'bg-zinc-100'}`}>
        <span className={`font-bold cursor-pointer flex items-center text-lg ${isMenuOpen ? 'bg-white py-4 px-4 rounded-md' : 'text-blue-500'}`} onClick={() => setMenuOpen(!isMenuOpen)}>
          {isMenuOpen ? <FaTimes className="w-6 h-6 mr-2" /> : <FaBars className="w-6 h-6 mr-2" />}
          {isMenuOpen && 'Seguindo'}
        </span>

        {isMenuOpen && (
          <div className="menu-overlay p-4 rounded-md">
            {data?.map((friendship: IFriendship, index: number) => (
              <div key={friendship.id} className={`flex flex-col gap-3 items-start mb-4 ${index < data.length - 1 ? 'border-black border-gray-300' : ''}`}>
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-md hover:bg-blue-400 text-white font-semibold ${isMenuOpen ? 'bg-blue-500' : 'bg-gray-400'}`}
                >
                  Unfollow
                </button>
              </div>
