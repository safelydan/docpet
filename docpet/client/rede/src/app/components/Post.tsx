...
function Post(props: { post: IPost }) {
  const { post_desc, img, username, userImg, created_at, id, userId } =
    props.post;
  ...
  const likesMutation = useMutation({
    mutationFn: async (newLikes: {}) => {
      if (liked) {
        await makeRequest
          .delete(`likes/?likes_post_id=${id}&likes_user_id=${user?.id}`)
          .then((res) => {
            setLiked(false);
            return res.data;
          });
  ...
  const commentQuery = useQuery<IComments[] | undefined>({
    ...
    queryFn: () =>
      makeRequest.get("comment/?post_id=" + id).then((res) => {
        return res.data.data;
      }),
    enabled: !!id,
  });
  ...
  return (
    <div className="w-full bg-white rounded-lg p-4 border border-gray-300 min-h-[300px]">
      <div className="overflow-hidden">
        <header className="flex gap-2 pb-4 border-b items-center relative">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs">
              {moment(created_at).fromNow().charAt(0).toUpperCase() +
                moment(created_at).fromNow().slice(1)}
            </span>
          </div>
        ...
        <div className="flex justify-between py-4 border-b">
            ...
            {likesQuery.data && likesQuery.data.length > 0 && (
              <>
                <div className="flex gap-1 items-center">
                  <span
                    className={`bg-red-500 w-6 h-6 text-white flex items-center justify-center rounded-full text-xs ${
                      liked ? "animate-like" : ""
                    }`}
                  >
                    <FaHeart />
          ...
          <button onClick={() => setShowComments(!showComments)}>
            {commentQuery.data &&
              ...
              `${commentQuery.data.length} comentários`}
          </button>
        </div>
        ...
        {showComments &&
          commentQuery.data?.map((comment, id) => (
